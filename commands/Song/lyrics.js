...
module.exports = {
    ...
    run: async (client, message, args, cmduser, text, prefix) => {
            ...
            if (!args[0] && !channel)  return message.channel.send(`:x: **Mày phải ở trong phòng**`);
            //send error if member is Deafed
            if(!args[0] && message.member.voice.selfDeaf) return message.channel.send(`:x: **Mày không thể gọi tao khi bị điếc**`);
