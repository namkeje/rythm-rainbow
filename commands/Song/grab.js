...
module.exports = {
  ...
  run: async (client, message, args, cmduser, text, prefix) => {
    ...
    if (!channel)  return message.channel.send(`:x: **Mày ph?i ? trong phòng**`);
    //send error if member is Deafed
    if(message.member.voice.selfDeaf) return message.channel.send(`:x: **Mày không th? g?i tao khi b? di?c**`);
    ...
    if(player && channel.id !== player.voiceChannel)
      return message.channel.send(`**:x: Mày ph?i ? cùng phòng v?i tao**`);
